 ...
 * 
 * To run:
  mpirun -n 9 --oversubscribe ./lr_coursework_mpi
 * 
 *
 *****************************************************************************/
...
int main() {
  ...
  MPI_Init(NULL, NULL); //Initilize the MPI library
  MPI_Comm_size(MPI_COMM_WORLD, &size); //Get the size of a communicator
  ...
  while(!minimum_found)
  {
	...
	//Calculate the time spent by the program from start to end.
	timedifference(&start, &finish, &timeelapsed);
	//Output the time spent by the program
